...
-(UIView *)initAddressView{
    ...
    titleLabel.textColor = COLOR_51;
    titleLabel.font = FONT_TEXT(15);
    [self.addAddressView addSubview:titleLabel];
    UIButton *cancelBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    cancelBtn.frame =CGRectMake(15 , 10, 30, 30);
    cancelBtn.tag = 1;
    cancelBtn.titleLabel.font=FONT_TEXT(14);
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [cancelBtn setTitleColor:COLOR_153 forState:UIControlStateNormal];
    ...
    selectBtn.frame =CGRectMake(CGRectGetMaxX(self.addAddressView.frame) - 40, 10, 30, 30);
    selectBtn.tag = 2;
    selectBtn.titleLabel.font=FONT_TEXT(14);
    [selectBtn setTitle:@"完成" forState:UIControlStateNormal];
    [selectBtn setTitleColor:UICOLOR_RGBA(25, 140, 255) forState:UIControlStateNormal];
    [selectBtn addTarget:self action:@selector(tapBtnAndSelectBtnClick) forControlEvents:UIControlEventTouchUpInside];
    [self.addAddressView addSubview:selectBtn];
    
    UIView * line = [[UIView alloc]initWithFrame:CGRectMake(0, 50, KScreenWidth, 0.5)];
    [self.addAddressView addSubview:line];
    line.backgroundColor=UICOLOR_RGBA(221, 221, 221);
...
- (void)tapBtnAndSelectBtnClick{
    NSString * address = @"";
    for ( NSString*city in self.titleMarr) {
        if (![city isEqualToString:@"请选择"]||![city isEqualToString:@"市辖区"]||![city isEqualToString:@"县"]) {
            address=[NSString stringWithFormat:@"%@%@",address,city];
        }
    }
    [self.delegate1 finishBtnClick:address titleID:self.titleIDMarr.lastObject longitude:self.longitude latitude:self.latitude];
...
-(void)setupTitleScrollView{
    ...
    lineView.backgroundColor = UICOLOR_RGBA(221, 221, 221);
...
-(void)setupAllTitle:(NSInteger)selectId{
    ...
    _lineLabel.backgroundColor = UICOLOR_RGBA(25, 140, 255);
    ...
    for (int i = 0; i < self.titleMarr.count ; i++) {
        ...
        [titleBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
         [titleBtn setTitleColor: UICOLOR_RGBA(25, 140, 255) forState:UIControlStateSelected];
...
-(void)setupOneTableView:(NSInteger)btnTag{
    ...
    if  (btnTag == 0) {
        [self getAddressMessageDataAddressID:1 provinceIdOrCityId:@"000000"];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 0) {
        TR_AreaModel * provinceModel = self.provinceMarr[indexPath.row];
        cell.nameLabel.text = provinceModel.criName;
        self.PCCTID = provinceModel.criCode;
    }
    else if (tableView.tag == 1) {
        TR_AreaModel *cityModel = self.cityMarr[indexPath.row];
        cell.nameLabel.text= cityModel.criName;
        self.PCCTID = cityModel.criCode;
    }
    else if (tableView.tag == 2){
        TR_AreaModel * countyModel  = self.countyMarr[indexPath.row];
        cell.nameLabel.text = countyModel.criName;
        self.PCCTID = countyModel.criCode;
    }
    else if (tableView.tag == 3){
        TR_AreaModel * townModel  = self.townMarr[indexPath.row];
        cell.nameLabel.text = townModel.criName;
        cell.imageIcon.hidden=YES;
        self.PCCTID = townModel.criCode;
    }
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 0 || tableView.tag == 1 || tableView.tag == 2){
        if (tableView.tag == 0){
            ...
            TR_AreaModel *provinceModel = self.provinceMarr[indexPath.row];
            NSString * provinceID = [NSString stringWithFormat:@"%@",provinceModel.criCode];
            //1. 修改选中ID
            ...
            //2.修改标题
              [self.titleMarr replaceObjectAtIndex:tableView.tag withObject:provinceModel.criName];
            //请求网络 添加市区
            ...
            self.selectIndexPath=nil;
            TR_AreaModel * cityModel = self.cityMarr[indexPath.row];
            NSString * cityID = [NSString stringWithFormat:@"%@",cityModel.criCode];
             [self.titleMarr replaceObjectAtIndex:tableView.tag withObject:cityModel.criName];
            //1. 修改选中ID
            ...
            self.selectIndexPath=nil;
            TR_AreaModel * countyModel = self.countyMarr[indexPath.row];
            NSString * countyID = [NSString stringWithFormat:@"%@",countyModel.criCode];
            [self.titleMarr replaceObjectAtIndex:tableView.tag withObject:countyModel.criName];
            //1. 修改选中ID
        ...
        if (self.selectIndexPath==nil) {
            ...
            AddressCell * cell_old = [tableView cellForRowAtIndexPath:self.selectIndexPath];
            cell_old.imageIcon.hidden=YES;
            AddressCell * cell_new = [tableView cellForRowAtIndexPath:indexPath];
            cell_new.imageIcon.hidden=NO;
            self.selectIndexPath=indexPath;
        }
      
        TR_AreaModel * townModel = self.townMarr[indexPath.row];
        NSString * townID = [NSString stringWithFormat:@"%@",townModel.criCode];
        [self.titleMarr replaceObjectAtIndex:tableView.tag withObject:townModel.criName];
        //1. 修改选中ID
        ...
        //最后的经纬度
        self.longitude =townModel.criLng;
        self.latitude = townModel.criLat;
    }
}
...
-(void)getAddressMessageDataAddressID:(NSInteger)addressID  provinceIdOrCityId: (NSString *)provinceIdOrCityId{
    WS(weakSelf);
    [self.viewModel getAreaList:provinceIdOrCityId completionBlock:^(BOOL flag, NSString *error) {
        if (flag) {
                    switch (addressID) {
                        case 1:
                            //拿到省列表
                        [self caseProvinceArr:weakSelf.viewModel.areaList];
                        break;
                        case 2:
                        //拿到市列表
                        [self caseCityArr:weakSelf.viewModel.areaList];
                        break;
                    case 3:
                        //拿到县列表
                        [self caseCountyArr:weakSelf.viewModel.areaList];
                        break;
                    case 4:
                        //拿到乡镇列表
                        [self caseTownArr:weakSelf.viewModel.areaList];
                    break;
                    default:
                    break;
                }
...
-(void)caseProvinceArr:(NSArray *)provinceArr{
    if (provinceArr.count > 0){
        ...
        [self.provinceMarr addObjectsFromArray:provinceArr];
    }else{
        [self tapBtnAndcancelBtnClick];
    }
}
-(void)caseCityArr:(NSArray *)cityArr{
    if (cityArr.count > 0){
        [self.cityMarr removeAllObjects];
         [self.cityMarr addObjectsFromArray:cityArr];
...
-(void)caseCountyArr:(NSArray *)countyArr{
    if (countyArr.count > 0){
        [self.countyMarr removeAllObjects];
         [self.countyMarr addObjectsFromArray:countyArr];
...
-(void)caseTownArr:(NSArray *)townArr{
    if (townArr.count > 0){
        ...
        [self.townMarr addObjectsFromArray:townArr];
